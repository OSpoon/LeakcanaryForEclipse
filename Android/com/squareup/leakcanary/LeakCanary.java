...
public final class LeakCanary {
	...
	private static final String GIT_SHA = "SPOOON";
	...
	/**
	 * Builder to create a customized {@link RefWatcher} with appropriate
	 * Android defaults.
	 */
	...
	/**
	 * If you build a {@link RefWatcher} with a {@link AndroidHeapDumper} that
	 * has a custom {@link LeakDirectoryProvider}, then you should also call
	 * this method to make sure the activity in charge of displaying leaks can
	 * find those on the file system.
	 */
	...
	public static String leakInfo(Context context, HeapDump heapDump, AnalysisResult result, boolean detailed) {
		...
		if (result.leakFound) {
			...
			// We duplicate the library version & Sha information because bug
			// reports often only contain
			// the stacktrace.
		...
		info += "* Reference Key: " + heapDump.referenceKey + "\n" + "* Device: " + Build.MANUFACTURER + " "
				...
				+ Build.VERSION.RELEASE + " API: " + Build.VERSION.SDK_INT + " LeakCanary: " + LIBRARY_VERSION + " "
				+ GIT_SHA + "\n" + "* Durations: watch=" + heapDump.watchDurationMs + "ms, gc=" + heapDump.gcDurationMs
				+ "ms, heap dump=" + heapDump.heapDumpDurationMs + "ms, analysis=" + result.analysisDurationMs + "ms"
				+ "\n" + detailedString;
